...
export default function Viewer() {
  ...
  // 2. Timer Loop & Sold Modal Trigger
  useEffect(() => {
    if (auction?.status === "ENDED") {
      setShowSoldModal(true);
      const timer = setTimeout(() => setShowSoldModal(false), 5000);
  ...
  const FinalLeaderboard = () => (
    <div className="w-full max-w-7xl animate-in fade-in zoom-in duration-1000 pb-20">
      <div className="text-center mb-16">
        ...
        <p className="text-zinc-400 font-black tracking-[0.4em] uppercase text-sm md:text-lg">
          Final Squads & Standings
        </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[1, 2, 3].map((i) => {
          ...
          return (
            <Card
              key={i}
              className="bg-black/40 border-orange-500/10 p-8 flex flex-col shadow-2xl"
            >
              <div className="flex justify-between items-end mb-6 border-b border-zinc-800 pb-4">
                <div>
                  <h3 className="text-2xl font-black text-orange-500 uppercase italic tracking-wide">
                    {team?.name || `Team ${i}`}
                  </h3>
                </div>
              ...
              <div className="space-y-3 overflow-y-auto max-h-[500px] pr-2 custom-scrollbar">
                {team?.players?.map((p, idx) => (
                  <div
                    key={idx}
                    className="flex justify-between items-center bg-zinc-900/40 p-4 rounded-2xl border border-zinc-800/60"
                  >
                    <div>
                      <p className="text-white font-bold text-base">{p.name}</p>
                      <p className="text-orange-400 text-[10px] font-black uppercase tracking-wider">
                        {p.role}
                      </p>
                    </div>
    ...
    <div className="relative min-h-screen bg-neutral-950 text-white p-6 md:p-8 flex flex-col overflow-hidden select-none">
      ...
      <div className="relative z-10 flex justify-between items-center mb-12 border-b border-orange-500/10 pb-6">
        <h1 className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter text-white">
          Bid Wars{" "}
          <span className="text-orange-500 text-xl md:text-2xl ml-2 font-black not-italic border-l border-zinc-800 pl-3">
            Viewer Arena
          </span>
        </h1>
        {auction?.status === "LIVE" && (
          <div className="flex items-center gap-3 bg-orange-500/10 border border-orange-500/20 px-5 py-1.5 rounded-full animate-pulse">
            ...
            <span className="text-orange-500 font-black uppercase text-xs tracking-widest">
              Live Arena Stream
            </span>
          </div>
        )}
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center w-full max-w-[1600px] mx-auto">
        {auction?.status === "LIVE" ? (
          <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-12 items-center">
            ...
            <div className="flex flex-col gap-6 w-full lg:col-span-1">
              <Card
                className={`p-8 bg-black/40 flex flex-col items-center justify-center backdrop-blur-xl transition-all duration-300 ${displayTime <= 3 ? "border-yellow-500 shadow-[0_0_30px_rgba(234,179,8,0.15)]" : "border-orange-500/10"}`}
              >
                <span
                  className={`text-8xl font-black leading-none tracking-tighter italic ${displayTime <= 3 ? "text-yellow-400" : "text-white"}`}
                >
                  {displayTime}s
                </span>
                <p className="text-zinc-500 font-black uppercase tracking-[0.35em] text-sm mt-4">
                  Remaining
                </p>
              </Card>
              ...
              <Card className="p-8 bg-zinc-900/30 border-orange-500/20 flex flex-col items-center justify-center backdrop-blur-xl">
                ...
                <div className="mt-5 bg-gradient-to-r from-orange-600 to-orange-700 text-white px-8 py-2.5 rounded-xl font-black uppercase tracking-wider text-base shadow-lg border border-orange-500/20 text-center w-full truncate">
          ...
          <div className="text-center animate-in fade-in slide-in-from-bottom-6 duration-700">
            ...
            {auction?.upcomingPlayer ? (
              ...
              <h3 className="text-3xl font-black text-zinc-600 uppercase italic tracking-wider">
                Preparing Next Deck
              </h3>
            )}
          </div>
        ...
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-6 mt-12 pt-8 border-t border-orange-500/10">
          {[1, 2, 3].map((i) => {
            ...
            return (
              <Card
                key={i}
                className="bg-black/40 border-orange-500/10 p-5 rounded-2xl backdrop-blur-sm flex flex-col"
              >
                <div className="flex justify-between items-center mb-3 border-b border-zinc-800 pb-2">
                  <div>
                    <h4 className="text-orange-500 font-black uppercase text-sm tracking-wide italic">
                      {team?.name || `Team ${i}`}
                    </h4>
                    <p className="text-zinc-500 text-[10px] font-bold uppercase mt-1">
                      Squad: {team?.players?.length || 0} / 11
                    </p>
                  </div>
